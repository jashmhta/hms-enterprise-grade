...
def run_bandit_scan():
    ...
    result = subprocess.run([
        'bandit', '-r', '/root/hms-enterprise-grade/backend',
        '-f', 'json', '-o', '/tmp/bandit_current.json'
...
class TestSecurityFixes:
    ...
    def test_issue_reduction(self, baseline_issues, run_bandit_scan):
        ...
        assert current_count < baseline_count, f'Issues increased from {baseline_count} to {current_count}'
        assert reduction_percent > 80, f'Reduction only {reduction_percent:.1f}% - target >80%'

        print(f'Security issues reduced from {baseline_count} to {current_count} ({reduction_percent:.1f}% improvement)')

    def test_high_critical_elimination(self, baseline_issues, run_bandit_scan):
        """Ensure all HIGH/CRITICAL issues are resolved."""
        baseline_high = sum(1 for issue in baseline_issues['results']
                            if issue['issue_severity'] in ['HIGH', 'CRITICAL'])

        current_scan = run_bandit_scan
        current_high = sum(1 for issue in current_scan['results']
                           if issue['issue_severity'] in ['HIGH', 'CRITICAL'])

        assert current_high == 0, f'{current_high} HIGH/CRITICAL issues remain - target: 0'
    ...
    def test_dangerous_patterns_eliminated(self, run_bandit_scan):
        ...
        dangerous_tests = ['B102', 'B307', 'B602', 'B605', 'B303', 'B324', 'B402', 'B302']

        current_scan = run_bandit_scan
        remaining_dangerous = sum(1 for issue in current_scan['results']
                                  if issue['test_id'] in dangerous_tests)

        assert remaining_dangerous == 0, f'{remaining_dangerous} dangerous patterns remain'
...
class TestHealthcareFunctionality:
    ...
    @pytest.mark.parametrize('service', ['appointments', 'billing', 'patients', 'lab', 'radiology', 'ehr'])
    def test_service_endpoints(self, service):
        ...
        ], cwd='/root/hms-enterprise-grade/backend', capture_output=True, text=True)

        # Verify service exists in URL patterns
        assert service in result.stdout.lower(), f'{service} service not found in URL patterns'
    ...
    def test_django_security_settings(self):
        ...
        assert len(missing_settings) == 0, f'Missing security settings: {missing_settings}'
...
class TestHIPAACompliance:
    ...
    def test_encryption_at_rest(self):
        ...
        assert len(exposed_envs) <= 1, f'{len(exposed_envs)} exposed environment files found'
    ...
    def test_secure_transmission(self):
        ...
        assert 'SESSION_COOKIE_SECURE = True' in content, 'Secure session cookies not enabled'
    ...
    def test_access_controls(self):
        ...
        for service in ['appointments', 'billing', 'patients', 'lab', 'radiology', 'ehr']:
            ...
            if os.path.exists(service_path):
                ...
                assert auth_found, f'No authentication imports found in {service} service'
...
class TestHealthcareDataIntegrity:
    ...
    def test_input_validation(self):
        ...
        for service in ['patients', 'billing', 'ehr']:
            ...
            if os.path.exists(service_path):
                ...
                assert validation_count / total_files >= 0.5, f'{service}: Only {validation_count}/{total_files} files have validation'
                print(f'{service}: {validation_count}/{total_files} files with validation')
    ...
    def test_pii_protection(self):
        ...
        if os.path.exists(patient_path):
            ...
            print(f'PII protection: {protection_count}/{total_files} patient files secured')
...
class TestPerformanceAndRollback:
    ...
    def test_backup_integrity(self):
        ...
        backup_files = list(Path('/root/hms-enterprise-grade/backend').rglob('*.py.backup'))

        # Should have backups from fix scripts
        assert len(backup_files) > 10, f'Only {len(backup_files)} backup files created - expected >10'
    ...
    def test_system_backup_exists(self):
        ...
        result = subprocess.run(['tar', '-tzf', str(latest_backup)], capture_output=True)
...
class TestComplianceReporting:
    ...
    def test_generate_compliance_report(self, run_bandit_scan):
        ...
        baseline_high = sum(1 for i in baseline_issues['results'] if i['issue_severity'] in ['HIGH', 'CRITICAL'])
        current_high = sum(1 for i in current_scan['results'] if i['issue_severity'] in ['HIGH', 'CRITICAL'])
...
"
        if len(list(Path('/root/hms-enterprise-grade/backend').rglob('*.py.backup'))) > 10:
...
"
        ...
        with open('/root/hms-enterprise-grade/docs/security/FINAL_COMPLIANCE_REPORT.md', 'w') as f:
            f.write(report)

        print('Final compliance report generated')
        assert os.path.exists('/root/hms-enterprise-grade/docs/security/FINAL_COMPLIANCE_REPORT.md')
