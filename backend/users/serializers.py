...
from .models import User, Department, UserPermissionGroup, UserSession, UserLoginHistory, UserCredential
...
class DepartmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'parent', 'is_active', 'budget', 'location', 'phone', 
            'email', 'user_count', 'subdepartments_count',
...
class UserCredentialSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            'id', 'credential_type', 'credential_name', 'issuing_organization',
            'credential_number', 'issue_date', 'expiry_date', 'is_active',
            'verification_status', 'document', 'notes', 'created_at', 'updated_at'
        ]
        extra_kwargs = {
            'credential_number': {'write_only': True}
        }
...
class UserSessionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserSession
        fields = [
            'id', 'session_key', 'ip_address', 'user_agent', 'location',
            'is_active', 'login_time', 'logout_time', 'last_activity', 'duration'
        ]
...
class UserLoginHistorySerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            'id', 'username_attempted', 'ip_address', 'user_agent',
            'success', 'failure_reason', 'timestamp', 'location'
        ]
...
class UserListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = [
            'id', 'username', 'employee_id', 'full_name', 'first_name', 'last_name',
            'email', 'role', 'status', 'department', 'department_name',
            'supervisor_name', 'hire_date', 'employment_type', 'last_login',
            'credentials_count', 'is_active', 'date_joined'
        ]
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = [
            'id', 'username', 'employee_id', 'full_name', 'first_name', 'middle_name',
            'last_name', 'suffix', 'email', 'role', 'status', 'department',
            'department_name', 'supervisor', 'supervisor_name', 'date_of_birth',
            'gender', 'work_phone', 'employment_type', 'hire_date', 'termination_date',
            'license_expiry', 'specialization', 'board_certification',
            'years_experience', 'mfa_enabled', 'background_check_date', 'drug_test_date',
            'hipaa_training_date', 'orientation_completed', 'last_activity',
            'avatar', 'bio', 'preferences', 'credentials', 'recent_sessions',
...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = [
            'username', 'password', 'password_confirm', 'employee_id',
            'first_name', 'middle_name', 'last_name', 'suffix', 'email',
            'role', 'hospital', 'department', 'supervisor', 'date_of_birth',
            'gender', 'personal_phone', 'work_phone', 'personal_email',
            'address_line1', 'address_line2', 'city', 'state', 'zip_code',
            'country', 'employment_type', 'hire_date', 'salary', 'hourly_rate',
            'license_number', 'license_expiry', 'specialization',
            'board_certification', 'years_experience'
        ]
        extra_kwargs = {
...
class UserUpdateSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            'email', 'role', 'status', 'department', 'supervisor',
            'date_of_birth', 'gender', 'personal_phone', 'work_phone',
            'personal_email', 'address_line1', 'address_line2', 'city',
            'state', 'zip_code', 'country', 'employment_type', 'hire_date',
            'termination_date', 'salary', 'hourly_rate', 'license_number',
            'license_expiry', 'specialization', 'board_certification',
            'years_experience', 'background_check_date', 'drug_test_date',
            'hipaa_training_date', 'orientation_completed', 'avatar', 'bio',
            'preferences'
        ]
        extra_kwargs = {
            'personal_phone': {'write_only': True},
...
class UserPermissionGroupSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'name', 'description', 'permissions', 'users',
            'permissions_count', 'users_count', 'is_active',
            'created_at', 'updated_at'
        ]
...
class CustomTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def get_token(cls, user):
        ...
        flags = ['enable_opd', 'enable_ipd', 'enable_diagnostics', 'enable_pharmacy', 'enable_accounting']
        hp = None
        try:
