...
from drf_spectacular.plumbing import (ResolvedComponent, build_array_type,
                                      build_object_type, follow_field_source,
                                      get_doc)
...
def _inject_enum_collision_fix(collection):
    ...
    if (
        not collection
        and "GisFeatureEnum" not in spectacular_settings.ENUM_NAME_OVERRIDES
    ):
        spectacular_settings.ENUM_NAME_OVERRIDES["GisFeatureEnum"] = ("Feature",)
    if (
        collection
        and "GisFeatureCollectionEnum" not in spectacular_settings.ENUM_NAME_OVERRIDES
    ):
        spectacular_settings.ENUM_NAME_OVERRIDES["GisFeatureCollectionEnum"] = (
            "FeatureCollection",
        )
...
class GeoFeatureModelSerializerExtension(OpenApiSerializerExtension):
    ...
    def map_geo_feature_model_serializer(self, serializer, base_schema):
        ...
        geo_properties = {"type": {"type": "string", "enum": ["Feature"]}}
        if serializer.Meta.id_field:
            geo_properties["id"] = base_schema["properties"].pop(
                serializer.Meta.id_field
            )

        geo_properties["geometry"] = map_geo_field(
            serializer, serializer.Meta.geo_field
        )
...
class GeometryFieldExtension(OpenApiSerializerFieldExtension):
    ...
    def map_serializer_field(self, auto_schema, direction):
        ...
        try:
            ...
            warn(
                f"Encountered an issue resolving field {self.target}. defaulting to generic object."
            )
            return {}
