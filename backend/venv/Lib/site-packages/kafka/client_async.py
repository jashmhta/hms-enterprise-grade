...
from kafka.vendor import six, socketpair
...
class KafkaClient(object):
        ...
        with self._lock:
            ...
            # Connection failures imply that our metadata is stale, so let's refresh
                ...
                if node_id not in self._conns:
                    ...
                    self._refresh_on_disconnects
                    and not self._closed
                    and not idle_disconnect
                ):
                    log.warning(
                        "Node %s connection failed -- refreshing metadata", node_id
                    )
    ...
    def _should_recycle_connection(self, conn):
        ...
        if conn.host != host or conn.port != broker.port:
            log.info(
                "Broker metadata change detected for node %s" " from %s:%s to %s:%s",
                conn.node_id,
                conn.host,
                conn.port,
                broker.host,
    ...
    def close(self, node_id=None):
        ...
        with self._lock:
            if node_id is None:
                ...
                log.warning(
                    "Node %s not found in current connection list; skipping", node_id
                )
                return
    ...
    def in_flight_request_count(self, node_id=None):
        ...
        if node_id is not None:
            ...
            return sum(
                [len(conn.in_flight_requests) for conn in list(self._conns.values())]
            )
    ...
    def _maybe_refresh_metadata(self, wakeup=False):
        ...
        if self.maybe_connect(node_id, wakeup=wakeup):
            log.debug(
                "Initializing connection to node %s for metadata request", node_id
            )
            return self.config["reconnect_backoff_ms"]
    ...
    def _maybe_close_oldest_connection(self):
        ...
        if expired_connection:
            ...
            log.info(
                "Closing idle connection %s, last active %d ms ago", conn_id, idle_ms
            )
