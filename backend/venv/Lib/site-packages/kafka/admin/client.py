...
from kafka.admin.acl_resource import (ACL, ACLFilter, ACLOperation,
                                      ACLPermissionType,
...
from kafka.errors import (IllegalArgumentError, IncompatibleBrokerVersion,
                          KafkaConfigurationError, NotControllerError,
                          UnrecognizedBrokerVersion)
from kafka.metrics import MetricConfig, Metrics
from kafka.protocol.admin import (AlterConfigsRequest, CreateAclsRequest,
                                  CreatePartitionsRequest, CreateTopicsRequest,
                                  DeleteAclsRequest, DeleteGroupsRequest,
                                  DeleteTopicsRequest, DescribeAclsRequest,
                                  DescribeConfigsRequest,
                                  DescribeGroupsRequest, ListGroupsRequest)
...
class KafkaAdminClient(object):
    ...
    def __init__(self, **configs):
        ...
        self._client = KafkaClient(
            metrics=self._metrics, metric_group_prefix="admin", **self.config
        )
    ...
    def _matching_api_version(self, operation):
        ...
        if version < min_version:
            ...
            raise IncompatibleBrokerVersion(
                "No version of the '{}' Kafka protocol is supported by both the client and broker.".format(
                    operation[0].__name__
                )
            )
        return version
    ...
    def _refresh_controller_id(self):
        ...
        if 1 <= version <= 6:
            ...
            if controller_version < (0, 10, 0):
                raise IncompatibleBrokerVersion(
                    "The controller appears to be running Kafka {}. KafkaAdminClient requires brokers >= 0.10.0.0.".format(
                        controller_version
                    )
                )
            ...
            raise UnrecognizedBrokerVersion(
                "Kafka Admin interface cannot determine the controller using MetadataRequest_v{}.".format(
                    version
                )
            )
    ...
    def _find_coordinator_id_send_request(self, group_id):
        ...
        if version <= 0:
            ...
            raise NotImplementedError(
                "Support for GroupCoordinatorRequest_v{} has not yet been added to KafkaAdminClient.".format(
                    version
                )
            )
    ...
    def _find_coordinator_id_process_response(self, response):
        ...
        if response.API_VERSION <= 0:
            ...
            if error_type is not Errors.NoError:
                ...
                raise error_type(
                    "FindCoordinatorRequest failed with response '{}'.".format(response)
                )
        else:
            raise NotImplementedError(
                "Support for FindCoordinatorRequest_v{} has not yet been added to KafkaAdminClient.".format(
    ...
    def create_topics(self, new_topics, timeout_ms=None, validate_only=False):
        ...
        if version == 0:
            if validate_only:
                raise IncompatibleBrokerVersion(
                    "validate_only requires CreateTopicsRequest >= v1, which is not supported by Kafka {}.".format(
            ...
            raise NotImplementedError(
                "Support for CreateTopics v{} has not yet been added to KafkaAdminClient.".format(
                    version
                )
            )
    ...
    def delete_topics(self, topics, timeout_ms=None):
        ...
        if version <= 3:
            ...
            raise NotImplementedError(
                "Support for DeleteTopics v{} has not yet been added to KafkaAdminClient.".format(
                    version
                )
            )
        return response
        ...
        if version <= 3:
            if auto_topic_creation:
                raise IncompatibleBrokerVersion(
                    ...
                    " is not supported by Kafka {}".format(self.config["api_version"])
                )

            request = MetadataRequest[version](topics=topics)
        elif version <= 5:
            request = MetadataRequest[version](
                topics=topics, allow_auto_topic_creation=auto_topic_creation
            )
    ...
    def _convert_describe_acls_response_to_acls(describe_response):
        ...
        for resources in describe_response.resources:
            if version == 0:
                ...
                raise NotImplementedError(
                    "Support for DescribeAcls Response v{} has not yet been added to KafkaAdmin.".format(
                        version
                    )
                )
            for acl in acls:
    ...
    def describe_acls(self, acl_filter):
        ...
        if version == 0:
            ...
            raise NotImplementedError(
                "Support for DescribeAcls v{} has not yet been added to KafkaAdmin.".format(
                    version
                )
            )
        ...
        if error_type is not Errors.NoError:
            ...
            raise error_type(
                "Request '{}' failed with response '{}'.".format(request, response)
            )
    ...
    def create_acls(self, acls):
        ...
        if version == 0:
            request = CreateAclsRequest[version](
                creations=[
                    self._convert_create_acls_resource_request_v0(acl) for acl in acls
                ]
            )
        elif version <= 1:
            request = CreateAclsRequest[version](
                creations=[
                    self._convert_create_acls_resource_request_v1(acl) for acl in acls
                ]
            )
        else:
            raise NotImplementedError(
                "Support for CreateAcls v{} has not yet been added to KafkaAdmin.".format(
                    version
                )
            )
    ...
    def _convert_delete_acls_response_to_matching_acls(acl_filters, delete_response):
        ...
        for i, filter_responses in enumerate(delete_response.filter_responses):
            ...
            for acl in matching_acls:
                ...
                acl_result_list.append(
                    (
                        conv_acl,
                        acl_error,
                    )
                )
            filter_result_list.append(
                (
                    acl_filters[i],
                    acl_result_list,
                    filter_error,
    ...
    def delete_acls(self, acl_filters):
        ...
        for acl in acl_filters:
            if not isinstance(acl, ACLFilter):
                raise IllegalArgumentError(
                    "acl_filters must contain ACLFilter type objects"
                )
        ...
        if version == 0:
            request = DeleteAclsRequest[version](
                filters=[
                    self._convert_delete_acls_resource_request_v0(acl)
                    for acl in acl_filters
                ]
            )
        elif version <= 1:
            request = DeleteAclsRequest[version](
                filters=[
                    self._convert_delete_acls_resource_request_v1(acl)
                    for acl in acl_filters
                ]
            )
        else:
            raise NotImplementedError(
                "Support for DeleteAcls v{} has not yet been added to KafkaAdmin.".format(
                    version
                )
            )
        ...
        return self._convert_delete_acls_response_to_matching_acls(
            acl_filters, response
        )
    ...
    def describe_configs(self, config_resources, include_synonyms=False):
        ...
        if version == 0:
            if include_synonyms:
                raise IncompatibleBrokerVersion(
                    "include_synonyms requires DescribeConfigsRequest >= v1, which is not supported by Kafka {}.".format(
                        self.config["api_version"]
                    )
                )
            ...
            if len(broker_resources) > 0:
                for broker_resource in broker_resources:
                    try:
                        ...
                        raise ValueError(
                            "Broker resource names must be an integer or a string represented integer"
                        )

                    futures.append(
                        self._send_request_to_node(
                            broker_id,
            ...
            if len(topic_resources) > 0:
                futures.append(
                    self._send_request_to_node(
                        self._client.least_loaded_node(),
                        DescribeConfigsRequest[version](
                            resources=topic_resources, include_synonyms=include_synonyms
                        ),
                    )
                )
        else:
            raise NotImplementedError(
                "Support for DescribeConfigs v{} has not yet been added to KafkaAdminClient.".format(
                    version
                )
            )
    ...
    def alter_configs(self, config_resources):
        ...
        if version <= 1:
            ...
            raise NotImplementedError(
                "Support for AlterConfigs v{} has not yet been added to KafkaAdminClient.".format(
                    version
                )
            )
    ...
    def _convert_create_partitions_request(topic_name, new_partitions):
        return (
            topic_name,
            (new_partitions.total_count, new_partitions.new_assignments),
        )
    ...
    def create_partitions(self, topic_partitions, timeout_ms=None, validate_only=False):
        ...
        if version <= 1:
            request = CreatePartitionsRequest[version](
                topic_partitions=[
                    self._convert_create_partitions_request(topic_name, new_partitions)
            ...
            raise NotImplementedError(
                "Support for CreatePartitions v{} has not yet been added to KafkaAdminClient.".format(
                    version
                )
            )
        return self._send_request_to_controller(request)
    ...
    def _describe_consumer_groups_send_request(
        self, group_id, group_coordinator_id, include_authorized_operations=False
    ):
        ...
        if version <= 2:
            ...
            raise NotImplementedError(
                "Support for DescribeGroupsRequest_v{} has not yet been added to KafkaAdminClient.".format(
                    version
                )
            )
    ...
    def _describe_consumer_groups_process_response(self, response):
        ...
        if response.API_VERSION <= 3:
            ...
            for response_field, response_name in zip(
                response.SCHEMA.fields, response.SCHEMA.names
            ):
                if isinstance(response_field, Array):
                    ...
                    for (
                        described_group_information,
                        group_information_name,
                        group_information_field,
                    ) in zip(
                        described_group,
                        described_groups_field_schema.names,
                        described_groups_field_schema.fields,
                    ):
                        ...
                        if isinstance(group_information_field, Array):
                            ...
                            for members in described_group_information:
                                member_information = []
                                for member, member_field, member_name in zip(
                                    members, member_schema.fields, member_schema.names
                                ):
                                    if protocol_type_is_consumer:
                                        if member_name == "member_metadata" and member:
                                            member_information.append(
                                                ConsumerProtocolMemberMetadata.decode(
                                                    member
                                                )
                                            )
                                        elif (
                                            member_name == "member_assignment"
                                            and member
                                        ):
                                            member_information.append(
                                                ConsumerProtocolMemberAssignment.decode(
                                                    member
                                                )
                                            )
                                        else:
                                            member_information.append(member)
                                member_info_tuple = MemberInformation._make(
                                    member_information
                                )
                    ...
                    group_description = GroupInformation._make(
                        described_group_information_list
                    )
            ...
            if error_type is not Errors.NoError:
                raise error_type(
                    "DescribeGroupsResponse failed with response '{}'.".format(response)
                )
        else:
            raise NotImplementedError(
                "Support for DescribeGroupsResponse_v{} has not yet been added to KafkaAdminClient.".format(
                    response.API_VERSION
                )
            )
    ...
    def describe_consumer_groups(
        self, group_ids, group_coordinator_id=None, include_authorized_operations=False
    ):
        ...
        if group_coordinator_id is not None:
            groups_coordinators = {
                group_id: group_coordinator_id for group_id in group_ids
            }
        else:
        ...
        futures = [
            self._describe_consumer_groups_send_request(
                group_id, coordinator_id, include_authorized_operations
            )
        ...
        for future in futures:
            response = future.value
            group_description = self._describe_consumer_groups_process_response(
                response
            )
    ...
    def _list_consumer_groups_send_request(self, broker_id):
        ...
        if version <= 2:
            ...
            raise NotImplementedError(
                "Support for ListGroupsRequest_v{} has not yet been added to KafkaAdminClient.".format(
                    version
                )
            )
    ...
    def _list_consumer_groups_process_response(self, response):
        ...
        if response.API_VERSION <= 2:
            ...
            if error_type is not Errors.NoError:
                raise error_type(
                    "ListGroupsRequest failed with response '{}'.".format(response)
                )
        else:
            raise NotImplementedError(
                "Support for ListGroupsResponse_v{} has not yet been added to KafkaAdminClient.".format(
                    response.API_VERSION
                )
            )
    ...
    def list_consumer_groups(self, broker_ids=None):
        ...
        for f in futures:
            ...
            consumer_groups.update(
                self._list_consumer_groups_process_response(response)
            )
        return list(consumer_groups)

    def _list_consumer_group_offsets_send_request(
        self, group_id, group_coordinator_id, partitions=None
    ):
        ...
        if version <= 3:
            ...
            raise NotImplementedError(
                "Support for OffsetFetchRequest_v{} has not yet been added to KafkaAdminClient.".format(
                    version
                )
            )
        return self._send_request_to_node(group_coordinator_id, request)
        ...
        if response.API_VERSION <= 3:
            ...
            if response.API_VERSION > 1:
                ...
                if error_type is not Errors.NoError:
                    # optionally we could retry if error_type.retriable
                    raise error_type(
                        "OffsetFetchResponse failed with response '{}'.".format(
                            response
                        )
                    )
            ...
            for topic, partitions in response.topics:
                for partition, offset, metadata, error_code in partitions:
                    ...
                    if error_type is not Errors.NoError:
                        raise error_type(
                            "Unable to fetch consumer group offsets for topic {}, partition {}".format(
                                topic, partition
                            )
                        )
                    offsets[TopicPartition(topic, partition)] = OffsetAndMetadata(
                        offset, metadata
                    )
        else:
            raise NotImplementedError(
                "Support for OffsetFetchResponse_v{} has not yet been added to KafkaAdminClient.".format(
                    response.API_VERSION
                )
            )
        return offsets
    ...
    def list_consumer_group_offsets(
        self, group_id, group_coordinator_id=None, partitions=None
    ):
    ...
    def delete_consumer_groups(self, group_ids, group_coordinator_id=None):
        ...
        if group_coordinator_id is not None:
            futures = [
                self._delete_consumer_groups_send_request(
                    group_ids, group_coordinator_id
                )
            ]
        else:
            coordinators_groups = defaultdict(list)
            for group_id, coordinator_id in self._find_coordinator_ids(
                group_ids
            ).items():
    ...
    def _convert_delete_groups_response(self, response):
        if response.API_VERSION <= 1:
            ...
            raise NotImplementedError(
                "Support for DeleteGroupsResponse_v{} has not yet been added to KafkaAdminClient.".format(
                    response.API_VERSION
                )
            )
    ...
    def _delete_consumer_groups_send_request(self, group_ids, group_coordinator_id):
        ...
        if version <= 1:
            ...
            raise NotImplementedError(
                "Support for DeleteGroupsRequest_v{} has not yet been added to KafkaAdminClient.".format(
                    version
                )
            )
        return self._send_request_to_node(group_coordinator_id, request)
