    ...
    def validate(self):
        if not isinstance(self.operation, ACLOperation):
            raise IllegalArgumentError(
                "operation must be an ACLOperation object, and cannot be ANY"
            )
        if not isinstance(self.permission_type, ACLPermissionType):
            raise IllegalArgumentError(
                "permission_type must be an ACLPermissionType object, and cannot be ANY"
            )
        if not isinstance(self.resource_pattern, ResourcePatternFilter):
            raise IllegalArgumentError(
                "resource_pattern must be a ResourcePatternFilter object"
            )
...
class ACL(ACLFilter):
    ...
    def __init__(self, principal, host, operation, permission_type, resource_pattern):
        super(ACL, self).__init__(
            principal, host, operation, permission_type, resource_pattern
        )
        self.validate()
    ...
    def validate(self):
        ...
        if not isinstance(self.resource_pattern, ResourcePattern):
            raise IllegalArgumentError(
                "resource_pattern must be a ResourcePattern object"
            )
...
class ResourcePatternFilter(object):
    ...
    def validate(self):
        ...
        if not isinstance(self.pattern_type, ACLResourcePatternType):
            raise IllegalArgumentError(
                "pattern_type must be an ACLResourcePatternType object"
            )
...
class ResourcePattern(ResourcePatternFilter):
    ...
    def __init__(
        self, resource_type, resource_name, pattern_type=ACLResourcePatternType.LITERAL
    ):
        super(ResourcePattern, self).__init__(
    ...
    def validate(self):
        ...
        if self.pattern_type in [
            ...
            ACLResourcePatternType.MATCH,
        ]:
            raise IllegalArgumentError(
                "pattern_type cannot be {} on a concrete ResourcePattern".format(
