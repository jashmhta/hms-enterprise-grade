...
from kafka.coordinator.assignors.sticky.partition_movements import \
    PartitionMovements
...
from kafka.coordinator.protocol import (ConsumerProtocolMemberAssignment,
                                        ConsumerProtocolMemberMetadata, Schema)
from kafka.protocol.struct import Struct
from kafka.protocol.types import Array, Int32, String
from kafka.structs import TopicPartition
from kafka.vendor import six

log = logging.getLogger(__name__)

ConsumerGenerationPair = namedtuple(
...
StickyAssignorMemberMetadataV1 = namedtuple(
    "StickyAssignorMemberMetadataV1", ["subscription", "partitions", "generation"]
)
...
class StickyAssignmentExecutor:
    ...
    def _init_current_assignments(self, members):
        ...
        for partitions, consumers in six.iteritems(
            sorted_partition_consumers_by_generation
        ):
    ...
    def _are_subscriptions_identical(self):
        ...
        if not has_identical_list_elements(
            list(six.itervalues(self.partition_to_all_potential_consumers))
        ):
            return False
        ...
        if not self.is_fresh_assignment and self._are_subscriptions_identical():
            ...
            while sorted_consumers:
                ...
                previous_partitions = set(
                    six.iterkeys(self.previous_assignment)
                ).intersection(set(remaining_partitions))
    ...
    def _initialize_current_subscriptions(self):
        self.sorted_current_subscriptions = SortedSet(
            iterable=[
                (consumer, tuple(partitions))
                for consumer, partitions in six.iteritems(self.current_assignment)
            ],
            key=subscriptions_comparator_key,
        )
    ...
    def _can_consumer_participate_in_reassignment(self, consumer):
        ...
        if current_assignment_size > max_assignment_size:
            log.error(
                "The consumer {} is assigned more partitions than the maximum possible.".format(
                    consumer
                )
            )
        if current_assignment_size < max_assignment_size:
        ...
        while True:
            ...
            for partition in reassignable_partitions:
                ...
                if len(self.partition_to_all_potential_consumers[partition]) <= 1:
                    log.error(
                        "Expected more than one potential consumer for partition {}".format(
                            partition
                        )
                    )
...
class StickyPartitionAssignor(AbstractPartitionAssignor):
    ...
    def metadata(cls, topics):
        if cls.member_assignment is None:
            ...
            log.debug(
                "Member assignment is available, generating the metadata: generation {}".format(
                    cls.generation
                )
            )
            ...
            for (
                topic_partition
            ) in cls.member_assignment:  # pylint: disable=not-an-iterable
                partitions_by_topic[topic_partition.topic].append(
                    topic_partition.partition
                )
            data = StickyAssignorUserDataV1(
                six.iteritems(partitions_by_topic), cls.generation
            )
