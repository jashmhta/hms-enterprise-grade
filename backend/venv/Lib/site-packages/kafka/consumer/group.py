...
class KafkaConsumer(six.Iterator):
    ...
    def __init__(self, *topics, **configs):
        ...
        if self.config["auto_offset_reset"] in deprecated:
            ...
            log.warning(
                "use auto_offset_reset=%s (%s is deprecated)",
                new_config,
            ...
            if "session_timeout_ms" not in configs:
                if "max_poll_interval_ms" in configs:
                    ...
                    self.config["session_timeout_ms"] = (
                        self.DEFAULT_SESSION_TIMEOUT_MS_0_9
                    )
    ...
    def _poll_once(self, timeout_ms, max_records, update_offsets=True):
        ...
        records, partial = self._fetcher.fetched_records(
        ...
        records, _ = self._fetcher.fetched_records(
            max_records, update_offsets=update_offsets
        )
        return records
    ...
    def seek(self, partition, offset):
        ...
        assert (
            partition in self._subscription.assigned_partitions()
    ...
    def seek_to_beginning(self, *partitions):
        ...
        if not partitions:
            ...
            for p in partitions:
                assert (
                    p in self._subscription.assigned_partitions()
                ), "Unassigned partition"
    ...
    def offsets_for_times(self, timestamps):
        ...
        if self.config["api_version"] <= (0, 10, 0):
            raise UnsupportedVersionError(
                "offsets_for_times API not supported for cluster version {}".format(
    ...
    def _message_generator(self):
        assert (
            self.assignment() or self.subscription() is not None
        ...
        while time.time() < self._consumer_timeout:
            ...
            poll_ms = min(
                (1000 * (self._consumer_timeout - time.time())),
