...
from kafka.protocol.offset import UNKNOWN_OFFSET, OffsetRequest, OffsetResetStrategy
...
CompletedFetch = collections.namedtuple(
    "CompletedFetch",
    [
        "topic_partition",
        "fetched_offset",
...
class Fetcher(six.Iterator):
    ...
    def __init__(self, client, subscriptions, metrics, **configs):
        ...
        self._next_partition_records = (
            None  # Holds a single PartitionRecords until fully consumed
        )
        self._iterator = None
    ...
    def reset_offsets_if_needed(self, partitions):
        ...
        for tp in partitions:
            ...
            if self._subscriptions.is_assigned(
                tp
            ) and self._subscriptions.is_offset_reset_needed(tp):
    ...
    def update_fetch_positions(self, partitions):
        ...
        for tp in partitions:
            if not self._subscriptions.is_assigned(tp):
                log.warning(
                    "partition %s is not assigned - skipping offset" " update", tp
                )
                continue
            elif self._subscriptions.is_fetchable(tp):
                log.warning(
                    "partition %s is still fetchable -- skipping offset" " update", tp
                )
                continue
    ...
    def _reset_offset(self, partition):
        ...
        log.debug(
            "Resetting offset for partition %s to %s offset.", partition, strategy
        )
        ...
        if partition in offsets:
            ...
            log.debug(
                "Could not find offset for partition %s since it is probably deleted"
                % (partition,)
            )
    ...
    def _retrieve_offsets(self, timestamps, timeout_ms=float("inf")):
        ...
        while remaining_ms > 0:
            ...
            if future.exception.invalid_metadata:
                ...
                if refresh_future.succeeded() and isinstance(
                    future.exception, Errors.StaleMetadata
                ):
                    log.debug(
                        "Stale metadata was raised, and we now have an updated metadata. Rechecking partition existence"
                    )
                    unknown_partition = future.exception.args[
                        0
                    ]  # TopicPartition from StaleMetadata
                    if (
                        self._client.cluster.leader_for_partition(unknown_partition)
                        is None
                    ):
                        log.debug(
                            "Removed partition %s from offsets retrieval"
                            % (unknown_partition,)
                        )
    ...
    def fetched_records(self, max_records=None, update_offsets=True):
        ...
        while records_remaining > 0:
            if not self._next_partition_records:
                ...
                records_remaining -= self._append(
                    drained,
                    self._next_partition_records,
                    records_remaining,
                    update_offsets,
    ...
    def _append(self, drained, part, max_records, update_offsets):
        ...
        if not self._subscriptions.is_assigned(tp):
            ...
            log.debug(
                ...
                " since it is no longer assigned",
                tp,
            )
        else:
            ...
            if not self._subscriptions.is_fetchable(tp):
                ...
                log.debug(
                    ...
                    " %s since it is no longer fetchable",
                    tp,
                )
                ...
                log.log(
                    ...
                    " partition %s and update position to %s",
                    position,
                    tp,
                    next_offset,
                )
                ...
                log.debug(
                    "Ignoring fetched records for %s at offset %s since"
        ...
        while self._next_partition_records or self._completed_fetches:
            ...
            for msg in self._next_partition_records.take():
                ...
                # Compressed messagesets may include earlier messages
                    ...
                    log.debug(
                        "Skipping message offset: %s (expecting %s)",
                        msg.offset,
            ...
            while batch is not None:
                ...
                try:
                    self._subscriptions.assignment[
                        tp
                    ].last_offset_from_message_batch = (
                ...
                for record in batch:
                    ...
                    key = self._deserialize(
                        self.config["key_deserializer"], tp.topic, record.key
                    )
                    value = self._deserialize(
                        self.config["value_deserializer"], tp.topic, record.value
                    )
                    headers = record.headers
                    header_size = (
                        sum(
                            len(h_key.encode("utf-8"))
                            + (len(h_val) if h_val is not None else 0)
            ...
            for partition_info in part_data:
                ...
                if error_type is Errors.NoError:
                    ...
                    log.debug(
                        ...
                        " message format version is before 0.10.0",
                        partition,
                    )
    ...
    def _create_fetch_requests(self):
        ...
        for partition in self._fetchable_partitions():
            ...
            if node_id is None or node_id == -1:
                log.debug(
                    "No leader found for partition %s." " Requesting metadata update",
                    partition,
                )
...
class FetchManagerMetrics(object):
    def __init__(self, metrics, prefix):
        ...
        self.records_fetch_lag.add(
            metrics.metric_name(
                "records-lag-max",
                self.group_name,
                "The maximum lag in terms of number of records for any partition in self window",
            ),
            Max(),
        )

        self.fetch_throttle_time_sensor = metrics.sensor("fetch-throttle-time")
        self.fetch_throttle_time_sensor.add(
            metrics.metric_name(
