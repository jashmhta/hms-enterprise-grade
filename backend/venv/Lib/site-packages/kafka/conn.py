...
from kafka.protocol.admin import DescribeAclsRequest_v2, SaslHandShakeRequest
...
class BrokerConnection(object):
    ...
    def __init__(self, host, port, afi, **configs):
        ...
        if self.config["send_buffer_bytes"] is not None:
            self.config["socket_options"].append(
                (socket.SOL_SOCKET, socket.SO_SNDBUF, self.config["send_buffer_bytes"])
            )

        assert (
            self.config["security_protocol"] in self.SECURITY_PROTOCOLS
        ...
        if self.config["security_protocol"] in ("SASL_PLAINTEXT", "SASL_SSL"):
            ...
            if self.config["sasl_mechanism"] in (
                "PLAIN",
                "SCRAM-SHA-256",
                "SCRAM-SHA-512",
            ):
                assert (
                    self.config["sasl_plain_username"] is not None
                ), "sasl_plain_username required for PLAIN or SCRAM sasl"
                assert (
                    self.config["sasl_plain_password"] is not None
                ), "sasl_plain_password required for PLAIN or SCRAM sasl"
            if self.config["sasl_mechanism"] == "GSSAPI":
                assert gssapi is not None, "GSSAPI lib not available"
                assert (
                    self.config["sasl_kerberos_service_name"] is not None
                ), "sasl_kerberos_service_name required for GSSAPI sasl"
            if self.config["sasl_mechanism"] == "OAUTHBEARER":
                ...
                assert callable(
                    getattr(token_provider, "token", None)
        ...
        self._protocol = KafkaProtocol(
            client_id=self.config["client_id"], api_version=self.config["api_version"]
        )
        ...
        if self.config["metrics"]:
            self._sensors = BrokerConnectionMetrics(
                self.config["metrics"], self.config["metric_group_prefix"], self.node_id
            )
    ...
    def _dns_lookup(self):
        ...
        if not self._gai:
            log.error(
                "DNS lookup failed for %s:%i (%s)", self.host, self.port, self.afi
            )
            return False
        return True
    ...
    def connect(self):
        ...
        if self.state is ConnectionStates.CONNECTING:
            ...
            # WSAEINVAL == 10022, but errno.WSAEINVAL is not available on non-win systems
                ...
                log.error(
                    "Connect attempt to %s returned error %s." " Disconnecting.",
                    self,
                    ret,
                )
    ...
    def _wrap_ssl(self):
        ...
        if self._ssl_context is None:
            ...
            if self.config["ssl_cafile"]:
                ...
                log.info(
                    "%s: Loading system default SSL CAs from %s",
                    self,
            ...
            if self.config["ssl_crlfile"]:
                if not hasattr(ssl, "VERIFY_CRL_CHECK_LEAF"):
                    raise RuntimeError(
                        "This version of Python does not support ssl_crlfile!"
                    )
                log.info(
            ...
            if self.config["ssl_ciphers"]:
                log.info(
                    "%s: Setting SSL Ciphers: %s", self, self.config["ssl_ciphers"]
                )
                self._ssl_context.set_ciphers(self.config["ssl_ciphers"])
        log.debug("%s: wrapping socket in ssl context", self)
        try:
            self._sock = self._ssl_context.wrap_socket(
                self._sock, server_hostname=self.host, do_handshake_on_connect=False
            )
    ...
    def _try_authenticate(self):
        assert self.config["api_version"] is None or self.config["api_version"] >= (
            0,
            10,
        )
    ...
    def _try_authenticate_plain(self, future):
        ...
        if data != b"\x00\x00\x00\x00":
            error = Errors.AuthenticationFailedError(
                "Unrecognized response during authentication"
            )
            return future.failure(error)

        log.info(
            "%s: Authenticated as %s via PLAIN",
            self,
            self.config["sasl_plain_username"],
        )
    ...
    def _try_authenticate_gssapi(self, future):
        ...
        gssapi_name = gssapi.Name(
            auth_id, name_type=gssapi.NameType.hostbased_service
        ...
        with self._lock:
            if not self._can_send_recv():
                ...
                try:
                    ...
                    client_ctx = gssapi.SecurityContext(
                        name=gssapi_name, usage="initiate"
                    )
                    received_token = None
                    ...
                    # by the server
                    msg = (
                        Int8.encode(SASL_QOP_AUTH & Int8.decode(io.BytesIO(msg[0:1])))
                        + msg[1:]
                    )
    ...
    def _build_oauth_client_request(self):
        ...
        return "n,,\x01auth=Bearer {}{}\x01\x01".format(
            token_provider.token(), self._token_extensions()
        )
    ...
    def _token_extensions(self):
        ...
        if (
            callable(getattr(token_provider, "extensions", None))
            and len(token_provider.extensions()) > 0
        ):
            msg = "\x01".join(
                ["{}={}".format(k, v) for k, v in token_provider.extensions().items()]
            )
            return "\x01" + msg
        else:
            return ""
    ...
    def _update_reconnect_backoff(self):
        ...
        if (
            ...
            self._failures += 1
            self._reconnect_backoff = self.config["reconnect_backoff_ms"] * 2 ** (
                self._failures - 1
            )
            self._reconnect_backoff = min(
                self._reconnect_backoff, self.config["reconnect_backoff_max_ms"]
            )
            self._reconnect_backoff *= uniform(0.8, 1.2)
            self._reconnect_backoff /= 1000.0
            log.debug(
                "%s: reconnect backoff %s after %s failures",
                self,
    ...
    def _send(self, request, blocking=True):
        ...
        with self._lock:
            ...
            if request.expect_response():
                sent_time = time.time()
                assert (
                    correlation_id not in self.in_flight_requests
    ...
    def recv(self):
        ...
        if not responses and self.requests_timed_out():
            ...
            self.close(
                error=Errors.RequestTimedOutError(
                    "Request timed out after %s ms" % self.config["request_timeout_ms"]
                )
            )
            return ()
        ...
        for i, (correlation_id, response) in enumerate(responses):
            try:
                ...
                self.close(
                    Errors.KafkaConnectionError("Received unrecognized correlation id")
                )
                return ()
            ...
            log.debug(
                "%s Response %d (%s ms): %s", self, correlation_id, latency_ms, response
            )
            ...
            while len(recvd) < self.config["sock_chunk_buffer_count"]:
                try:
                    ...
                    if six.PY2 and e.errno == errno.EWOULDBLOCK:
                        break
                    log.exception(
                        "%s: Error receiving network data" " closing socket", self
                    )
    ...
    def requests_timed_out(self):
        with self._lock:
            if self.in_flight_requests:
                ...
                oldest_at = min(map(get_timestamp, self.in_flight_requests.values()))
    ...
    def get_api_versions(self):
        ...
        if version < (0, 10, 0):
            raise Errors.UnsupportedVersionError(
                "ApiVersion not supported by cluster version {} < 0.10.0".format(
                    version
                )
            )
    ...
    def check_version(self, timeout=2, strict=False, topics=[]):
        ...
        for version, request in test_cases:
            ...
            if strict:
                ...
                # id mismatch
                elif isinstance(f.exception, Errors.CorrelationIdError) and version == (
                    0,
                    10,
                ):
                    pass
            ...
            log.info(
                "Broker is not v%s -- it did not recognize %s",
                version,
...
class BrokerConnectionMetrics(object):
    def __init__(self, metrics, metric_group_prefix, node_id):
        ...
        if not all_conns_transferred:
            ...
            bytes_sent = metrics.sensor("bytes-sent", parents=[bytes_transferred])
            bytes_sent.add(
                metrics.metric_name(
                    "outgoing-byte-rate",
                    metric_group_name,
            ...
            bytes_received = metrics.sensor(
                "bytes-received", parents=[bytes_transferred]
            )
            bytes_received.add(
                metrics.metric_name(
                    "incoming-byte-rate",
        ...
        if not node_sensor:
            ...
            bytes_sent = metrics.sensor(
                node_str + ".bytes-sent", parents=[metrics.get_sensor("bytes-sent")]
            )
            bytes_sent.add(
                metrics.metric_name(
                    "outgoing-byte-rate",
            ...
            request_time = metrics.sensor(
                node_str + ".latency", parents=[metrics.get_sensor("request-latency")]
            )
            request_time.add(
                metrics.metric_name(
                    "request-latency-avg",
...
def dns_lookup(host, port, afi=socket.AF_UNSPEC):
    ...
    try:
        return list(
            filter(
                is_inet_4_or_6, socket.getaddrinfo(host, port, afi, socket.SOCK_STREAM)
            )
        )
    except socket.gaierror as ex:
        log.warning(
            "DNS lookup failed for %s:%d,"
