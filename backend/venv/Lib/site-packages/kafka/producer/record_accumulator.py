...
class ProducerBatch(object):
    ...
    def try_append(self, timestamp_ms, key, value, headers):
        ...
        future = FutureRecordMetadata(
            ...
            (
                sum(len(h_key.encode("utf-8")) + len(h_val) for h_key, h_val in headers)
                if headers
                else -1
            ),
        )
    ...
    def done(
        ...
        log.log(
            level,
            "Produced messages to topic-partition %s with base offset"
            " %s log start offset %s and error %s.",
    ...
    def maybe_expire(self, request_timeout_ms, retry_backoff_ms, linger_ms, is_full):
        ...
        if not self.in_retry() and is_full and timeout < since_append:
            ...
            error = "%d seconds have passed since batch creation plus linger time" % (
                since_ready,
            )
        elif self.in_retry() and timeout < since_backoff:
            error = "%d seconds have passed since last attempt plus backoff time" % (
                since_backoff,
            )
...
class RecordAccumulator(object):
    ...
    def append(
        ...
        try:
            ...
            log.debug(
                "Allocating a new %d byte message buffer for %s", size, tp
            )  # trace
    ...
    def reenqueue(self, batch):
        ...
        batch.set_retry()
        assert (
            batch.topic_partition in self._tp_locks
        ), "TopicPartition not in locks dict"
    ...
    def ready(self, cluster):
        ...
        for tp in partitions:
            ...
            with self._tp_locks[tp]:
                ...
                backing_off = bool(
                    batch.attempts > 0 and batch.last_attempt + retry_backoff > now
                )
            ...
            while True:
                ...
                if tp in self._batches and tp not in self.muted:
                    with self._tp_locks[tp]:
                        ...
                        if dq:
                            first = dq[0]
                            backoff = bool(first.attempts > 0) and bool(
                                first.last_attempt
    ...
    def await_flush_completion(self, timeout=None):
        ...
        try:
            for batch in self._incomplete.all():
                log.debug(
                    "Waiting on produce to %s", batch.produce_future.topic_partition
                )
