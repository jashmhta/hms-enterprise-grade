...
class Sender(threading.Thread):
    ...
    def run(self):
        ...
        log.debug(
            "Beginning shutdown of Kafka producer I/O thread, sending"
            " remaining records."
        )
        ...
        while not self._force_close and (
            self._accumulator.has_unsent() or self._client.in_flight_request_count() > 0
        ):
            try:
                self.run_once()
            except Exception:
    ...
    def run_once(self):
        ...
        for node in list(ready_nodes):
            if not self._client.is_ready(node):
                log.debug(
                    "Node %s not ready; delaying produce of accumulated batch", node
                )
                self._client.maybe_connect(node, wakeup=False)
                ready_nodes.remove(node)
                not_ready_timeout = min(
                    not_ready_timeout, self._client.connection_delay(node)
                )
        ...
        for expired_batch in expired_batches:
            self._sensors.record_errors(
                expired_batch.topic_partition.topic, expired_batch.record_count
            )
        ...
        if ready_nodes:
            ...
            log.debug(
                "Created %d produce requests: %s", len(requests), requests
            )  # trace
            poll_timeout_ms = 0
        ...
        for node_id, request in six.iteritems(requests):
            ...
            (
                ...
                .add_errback(self._failed_produce, batches, node_id)
            )
    ...
    def _failed_produce(self, batches, node_id, error):
        log.debug(
            "Error sending produce request to node %d: %s", node_id, error
        )  # trace
            ...
            for topic, partitions in response.topics:
                for partition_info in partitions:
                    ...
                    if response.API_VERSION < 2:
                        ...
                        partition, error_code, offset, ts, log_start_offset = (
                            partition_info
                        )
                    else:
                    ...
                    self._complete_batch(
                        batch, error, offset, ts, log_start_offset, global_error
                    )

            if response.API_VERSION > 0:
                self._sensors.record_throttle_time(
                    response.throttle_time_ms, node=node_id
                )
    ...
    def _complete_batch(
        ...
        if error is not None and self._can_retry(batch, error):
            # retry
            log.warning(
                "Got error produce response on topic-partition %s,"
            ...
            self._sensors.record_retries(
                batch.topic_partition.topic, batch.record_count
            )
        else:
    ...
    def _can_retry(self, batch, error):
        ...
        """
        return batch.attempts < self.config["retries"] and getattr(
    ...
    def _create_produce_requests(self, collated):
        ...
        for node_id, batches in six.iteritems(collated):
            requests[node_id] = self._produce_request(
                node_id, self.config["acks"], self.config["request_timeout_ms"], batches
            )
        return requests
...
class SenderMetrics(object):
    ...
    def __init__(self, metrics, client, metadata):
        ...
        self.add_metric(
            "metadata-age",
            AnonMeasurable(
                lambda _, now: (now - self._metadata._last_successful_refresh_ms) / 1000
            ),
