...
from .serializers import MedicationSerializer, PrescriptionSerializer, InventoryTransactionSerializer
...
class MedicationViewSet(TenantScopedViewSet):
    ...
    def auto_reorder(self, request):
        ...
        for m in qs:
            try:
                if provider_url:
                    requests.post(provider_url, json={ 'item': m.name, 'qty': int(m.min_stock_level) - int(m.stock_quantity) + 1 }, headers={'Authorization': f'Bearer {token}' } if token else {}, timeout=5)
