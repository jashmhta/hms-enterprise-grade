...
class Bill(TenantModel):
	...
	status = models.CharField(max_length=16, default='DUE', choices=[('DUE','Due'),('PARTIAL','Partial'),('PAID','Paid')])
	insurance_claim_status = models.CharField(max_length=32, default='NOT_SUBMITTED', choices=[('NOT_SUBMITTED','Not Submitted'),('SUBMITTED','Submitted'),('RECEIVED','Received')])
	...
	def recalc(self):
		...
		self.status = 'PAID' if paid >= self.net_cents and self.net_cents > 0 else ('PARTIAL' if 0 < paid < self.net_cents else 'DUE')
