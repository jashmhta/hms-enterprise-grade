...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for code, name, price in services:
            obj, created = ServiceCatalog.objects.get_or_create(hospital=hospital, code=code, defaults={'name': name, 'price_cents': price})
            self.stdout.write(f"Service {code}: {'created' if created else 'exists'}")

        # Seed default shift
        day_shift, created = Shift.objects.get_or_create(hospital=hospital, name='Day', defaults={'start_time': '09:00', 'end_time': '17:00'})
