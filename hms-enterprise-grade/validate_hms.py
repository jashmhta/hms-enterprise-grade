...
def validate_hms_modules():
    ...
    required_modules = [
        "Patient Registration", "OPD Management", "IPD Management", "Operation Theatre", 
        "Emergency Department", "Pharmacy Management", "Laboratory Management", "Radiology Management", 
        "Blood Bank Management", "Insurance/TPA Management", "Billing & Invoicing", 
        "Role-Based Access Control", "HR and Payroll", "Housekeeping and Maintenance", 
        "Biomedical Equipment", "Dietary Management", "Ambulance Management", 
        "Patient Portal", "Doctor Portal", "E-Prescription", "Notification System", 
        "Feedback & Complaint Management", "Marketing CRM", "Analytics and Reporting", 
        "Medical Records Department", "NABH / JCI Compliance", "Advanced Backup and Disaster Recovery", 
    ...
    for module, patterns in module_mapping.items():
        ...
        if backend_path.exists():
            for pattern in patterns:
                backend_dirs = [d for d in backend_path.iterdir() if d.is_dir() and pattern in d.name.lower()]
                if backend_dirs:
                    found = True
        ...
        if services_path.exists():
            for pattern in patterns:
                service_dirs = [d for d in services_path.iterdir() if d.is_dir() and pattern in d.name.lower()]
                if service_dirs:
                    found = True
