...
class AvailableSlotsTest(TestCase):
    def setUp(self):
        ...
        User = get_user_model()
        self.doctor = User.objects.create_user(username='doc', password='x', role=UserRole.DOCTOR, hospital=self.h)
        # Create a patient record
        self.patient = Patient.objects.create(hospital=self.h, first_name='Pat', last_name='Ient')
    ...
    def test_slots(self):
        ...
        Appointment.objects.create(hospital=self.h, patient=self.patient, doctor=self.doctor, start_at=start, end_at=end, status=AppointmentStatus.SCHEDULED)
        from rest_framework.test import APIClient
        client = APIClient()
        client.force_authenticate(user=self.doctor)
        res = client.get(f'/api/appointments/available_slots/?doctor={self.doctor.id}&date={self.target_date.isoformat()}')
