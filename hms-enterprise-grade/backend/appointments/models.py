...
class Appointment(TenantModel):
    ...
    primary_provider = models.ForeignKey('users.User', on_delete=models.CASCADE, related_name='primary_appointments', null=True)
    additional_providers = models.ManyToManyField('users.User', blank=True, related_name='additional_appointments')
    
    # Appointment Details
    appointment_type = models.CharField(max_length=20, choices=AppointmentType.choices, default=AppointmentType.ROUTINE)
    ...
    # Status & Priority
    status = models.CharField(max_length=16, choices=AppointmentStatus.choices, default=AppointmentStatus.SCHEDULED)
    ...
    recurrence_pattern = models.CharField(max_length=15, choices=RecurrencePattern.choices, default=RecurrencePattern.NONE)
...
class WaitList(TenantModel):
    ...
    created_by = models.ForeignKey('users.User', on_delete=models.SET_NULL, null=True, related_name='created_waitlist_entries')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
