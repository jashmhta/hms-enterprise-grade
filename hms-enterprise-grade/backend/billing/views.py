...
from .serializers import BillSerializer, BillLineItemSerializer, PaymentSerializer, ServiceCatalogSerializer, DepartmentBudgetSerializer
...
class BillViewSet(TenantScopedViewSet):
    ...
    queryset = Bill.objects.select_related('patient', 'appointment').prefetch_related('items', 'payments').all()
    ...
    def set_discount(self, request, pk=None):
        ...
        if opa_url:
            try:
                d = requests.post(f"{opa_url}/v1/data/hms/allow", json={"input": {"path": "/api/billing/bills/set_discount", "role": getattr(user, 'role', None)}}, timeout=2)
...
class AccountingViewSet(TenantScopedViewSet):
    ...
    def export_bills_csv(self, request):
        ...
        for b in qs:
            writer.writerow([b.id, b.patient_id, b.total_cents, b.discount_cents, b.net_cents, b.paid_cents, b.status])
        return response_obj
    ...
    def export_bills_xlsx(self, request):
        ...
        for b in qs:
            ws.append([b.id, b.patient_id, b.total_cents, b.discount_cents, b.net_cents, b.paid_cents, b.status])
    ...
    def consolidated_pl(self, request):
        ...
        return response.Response({ 'revenue_cents': int(revenue), 'depreciation_cents': int(depreciation), 'profit_cents': int(pl), 'beds': total_beds, 'occupied_beds': occupied_beds, 'occupancy_rate': occupancy_rate })
    ...
    def outsourced_breakdown(self, request):
        qs = self.get_queryset()
        outsourced = BillLineItem.objects.filter(bill__in=qs, is_outsourced=True).aggregate(total=Sum('amount_cents'))['total'] or 0
        inhouse = BillLineItem.objects.filter(bill__in=qs, is_outsourced=False).aggregate(total=Sum('amount_cents'))['total'] or 0
...
class DepartmentBudgetViewSet(TenantScopedViewSet):
    ...
    def utilization(self, request):
        ...
        return response.Response({ 'budget_cents': int(budget), 'spend_cents': int(spend), 'utilization_pct': pct })