...
def send_appointment_reminders():
    now = timezone.now()
    upcoming = Appointment.objects.filter(status=AppointmentStatus.SCHEDULED, start_at__range=(now, now + timezone.timedelta(hours=24)))
...
def check_low_stock_and_notify():
    ...
    if low.exists():
        ...
        send_mail("Low stock alert", f"Low stock items: {names}", None, ["admin@example.com"], fail_silently=True)