...
class Patient(TenantModel):
    ...
    # Diversity & Inclusion
    race = models.CharField(max_length=25, choices=RaceChoices.choices, default=RaceChoices.PREFER_NOT_TO_SAY)
    ethnicity = models.CharField(max_length=25, choices=EthnicityChoices.choices, default=EthnicityChoices.PREFER_NOT_TO_SAY)
    religion = models.CharField(max_length=25, choices=ReligionChoices.choices, default=ReligionChoices.PREFER_NOT_TO_SAY)
    preferred_language = models.CharField(max_length=10, choices=PreferredLanguage.choices, default=PreferredLanguage.ENGLISH)
    ...
    # System Fields
    last_updated_by = models.ForeignKey('users.User', on_delete=models.SET_NULL, null=True, related_name='updated_patients')
    ...
    def get_age_at_date(self, reference_date):
        if not self.date_of_birth:
            return None
        return reference_date.year - self.date_of_birth.year - ((reference_date.month, reference_date.day) < (self.date_of_birth.month, self.date_of_birth.day))
