...
class SubscriptionState(object):
    ...
    def __init__(self, offset_reset_strategy="earliest"):
        ...
        """
        try:
            offset_reset_strategy = getattr(
                OffsetResetStrategy, offset_reset_strategy.upper()
            )
    ...
    def _ensure_valid_topic_name(self, topic):
        ...
        if len(topic) > self._MAX_NAME_LENGTH:
            raise ValueError(
                'Topic name is illegal, it can\'t be longer than {0} characters, topic: "{1}"'.format(
                    self._MAX_NAME_LENGTH, topic
                )
            )
        if not self._TOPIC_LEGAL_CHARS.match(topic):
            raise ValueError(
                'Topic name "{0}" is illegal, it contains a character other than ASCII alphanumerics, ".", "_" and "-"'.format(
                    topic
                )
            )
    ...
    def is_fetchable(self, partition):
        return (
            partition in self.assignment and self.assignment[partition].is_fetchable()
        )
