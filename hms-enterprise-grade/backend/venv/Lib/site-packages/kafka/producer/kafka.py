...
class KafkaProducer(object):
    ...
    def close(self, timeout=None):
        ...
        if timeout > 0:
            if invoked_from_callback:
                log.warning(
                    "Overriding close timeout %s secs to 0 in order to"
    ...
    def send(
        ...
        """
        assert value is not None or self.config["api_version"] >= (
            0,
            8,
            1,
        ...
        try:
            ...
            log.debug(
                "Sending (key=%r value=%r headers=%r) to %s", key, value, headers, tp
            )
            ...
            return FutureRecordMetadata(
                ...
                (
                    sum(
                        len(h_key.encode("utf-8")) + len(h_value)
