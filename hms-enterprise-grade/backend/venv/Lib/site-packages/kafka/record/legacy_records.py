...
from kafka.codec import (gzip_decode, gzip_encode, lz4_decode,
                         lz4_decode_old_kafka, lz4_encode,
                         lz4_encode_old_kafka, snappy_decode, snappy_encode)
...
class LegacyRecordBatchBuilder(ABCRecordBatchBuilder, LegacyRecordBase):
    ...
    def append(self, offset, timestamp, key, value, headers=None):
        ...
        if self._magic == 0:
            ...
            raise TypeError(
                "`timestamp` should be int, but {} provided".format(type(timestamp))
            )
    ...
    def _encode_msg(self, start_pos, offset, timestamp, key, value, attributes=0):
        ...
        if magic == 0:
            self.HEADER_STRUCT_V0.pack_into(
                buf, start_pos, offset, length, 0, magic, attributes
            )
        else:
            self.HEADER_STRUCT_V1.pack_into(
                buf, start_pos, offset, length, 0, magic, attributes, timestamp
            )
