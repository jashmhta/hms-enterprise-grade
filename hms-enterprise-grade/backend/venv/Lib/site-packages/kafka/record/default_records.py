...
from kafka.codec import (gzip_decode, gzip_encode, lz4_decode, lz4_encode,
                         snappy_decode, snappy_encode, zstd_decode,
                         zstd_encode)
...
from kafka.record.util import (calc_crc32c, decode_varint, encode_varint,
                               size_of_varint)
    ...
    def _read_msg(self, decode_varint=decode_varint):
        ...
        if header_count < 0:
            raise CorruptRecordException(
                "Found invalid number of record " "headers {}".format(header_count)
            )
        headers = []
        while header_count:
...
class DefaultRecordMetadata(object):
    ...
    def __repr__(self):
        return "DefaultRecordMetadata(offset={!r}, size={!r}, timestamp={!r})".format(
            self._offset, self._size, self._timestamp
        )
