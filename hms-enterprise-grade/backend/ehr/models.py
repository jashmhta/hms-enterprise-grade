...
class Encounter(TenantModel):
    ...
    primary_physician = models.ForeignKey('users.User', on_delete=models.CASCADE, related_name='primary_encounters', null=True)
    attending_physician = models.ForeignKey('users.User', on_delete=models.SET_NULL, null=True, blank=True, related_name='attending_encounters')
    ...
    encounter_status = models.CharField(max_length=15, choices=EncounterStatus.choices, default=EncounterStatus.SCHEDULED)
    ...
    prescription_text = EncryptedTextField(blank=True)  # Keep for backward compatibility
    ...
    # System Fields
    created_by = models.ForeignKey('users.User', on_delete=models.SET_NULL, null=True, related_name='created_encounters')
