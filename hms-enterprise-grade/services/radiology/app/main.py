...
DATABASE_URL = os.getenv("RADIOLOGY_DATABASE_URL", os.getenv("DATABASE_URL", "sqlite:///./radiology.db"))
...
def create_order(payload: RadiologyOrder, claims: dict = Depends(require_auth), db: Session = Depends(get_db)):
    ensure_role(claims, { 'SUPER_ADMIN', 'HOSPITAL_ADMIN', 'DOCTOR', 'RECEPTIONIST' })
    ensure_module_enabled(claims, 'enable_diagnostics')
    row = OrderModel(patient_id=payload.patient_id, study_type=payload.study_type, priority=payload.priority)
    db.add(row)
    db.commit()
    db.refresh(row)
    return {"id": row.id, "patient_id": row.patient_id, "study_type": row.study_type, "priority": row.priority}
...
def list_orders(claims: dict = Depends(require_auth), db: Session = Depends(get_db)):
    ...
    return [{"id": r.id, "patient_id": r.patient_id, "study_type": r.study_type, "priority": r.priority} for r in rows]

@app.post("/api/radiology/report")
def submit_report(payload: RadiologyReport, claims: dict = Depends(require_auth), db: Session = Depends(get_db)):
...
def kpi(claims: dict = Depends(require_auth), db: Session = Depends(get_db)):
    ...
    if opa:
        try:
            d = requests.post(f"{opa}/v1/data/hms/allow", json={"input": {"path": "/api/radiology/kpi"}}, timeout=2)
