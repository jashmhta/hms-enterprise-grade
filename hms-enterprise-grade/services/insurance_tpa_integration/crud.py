...
def get_insurance_provider(db: Session, provider_id: int):
    return db.query(models.InsuranceProvider).filter(models.InsuranceProvider.id == provider_id).first()
...
def get_patient_policies(db: Session, patient_id: int):
    return db.query(models.InsurancePolicy).filter(models.InsurancePolicy.patient_id == patient_id).all()
...
def get_patient_claims(db: Session, patient_id: int):
    return db.query(models.InsuranceClaim).filter(models.InsuranceClaim.patient_id == patient_id).all()

def update_claim_status(db: Session, claim_id: int, status: str, approved_amount: float = None):
...
def get_claim_transactions(db: Session, claim_id: int):
    return db.query(models.TPATransaction).filter(models.TPATransaction.claim_id == claim_id).all()
...
def get_claim_payments(db: Session, claim_id: int):
    return db.query(models.PaymentRecord).filter(models.PaymentRecord.claim_id == claim_id).all()

def check_insurance_eligibility(db: Session, patient_id: int, policy_id: int, service_date: datetime):
    ...
    return {
        ...
        "requires_preauth": policy.coverage_details.get("requires_preauthorization", False),
...
def submit_claim_to_tpa(db: Session, claim_id: int, tpa_provider_id: int):
    ...
    transaction = create_tpa_transaction(db, schemas.TPATransactionCreate(**transaction_data))
    return transaction
