...
async def create_provider(provider: schemas.InsuranceProviderCreate, db: Session = Depends(get_db)):
...
async def create_policy(policy: schemas.InsurancePolicyCreate, db: Session = Depends(get_db)):
...
@app.get("/patients/{patient_id}/policies", response_model=List[schemas.InsurancePolicy])
...
async def create_claim(claim: schemas.InsuranceClaimCreate, db: Session = Depends(get_db)):
...
async def update_claim_status(claim_id: int, status_update: dict, db: Session = Depends(get_db)):
...
async def check_eligibility(request: schemas.EligibilityRequest, db: Session = Depends(get_db)):
    result = crud.check_insurance_eligibility(db, request.patient_id, request.policy_id, request.service_date)
    if not result:
        raise HTTPException(status_code=404, detail="Eligibility check failed")
    return result

@app.post("/claims/{claim_id}/submit")
async def submit_claim_to_tpa(claim_id: int, tpa_provider_id: int, db: Session = Depends(get_db)):
...
async def create_payment(payment: schemas.PaymentRecordCreate, db: Session = Depends(get_db)):
