...
def get_blood_units_by_type(db: Session, blood_type: schemas.BloodType, rh_factor: str):
    return db.query(models.BloodUnit).filter(
...
def get_transfusion_request(db: Session, request_id: int):
    return db.query(models.TransfusionRequest).filter(models.TransfusionRequest.id == request_id).first()

def update_transfusion_request_status(db: Session, request_id: int, status: str, approved_units: int = None):
    request = db.query(models.TransfusionRequest).filter(models.TransfusionRequest.id == request_id).first()
    if request:
...
def get_transfusion(db: Session, transfusion_id: int):
    return db.query(models.Transfusion).filter(models.Transfusion.id == transfusion_id).first()
...
def get_blood_tests_for_unit(db: Session, blood_unit_id: int):
    return db.query(models.BloodTest).filter(models.BloodTest.blood_unit_id == blood_unit_id).all()
...
def get_active_alerts(db: Session):
    return db.query(models.InventoryAlert).filter(models.InventoryAlert.is_resolved == False).all()

def resolve_alert(db: Session, alert_id: int, resolved_by: str):
    alert = db.query(models.InventoryAlert).filter(models.InventoryAlert.id == alert_id).first()
    if alert:
