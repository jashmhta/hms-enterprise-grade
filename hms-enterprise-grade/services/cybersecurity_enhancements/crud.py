...
def get_security_events(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.SecurityEvent).order_by(desc(models.SecurityEvent.timestamp)).offset(skip).limit(limit).all()
...
def get_audit_logs(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.AuditLog).order_by(desc(models.AuditLog.timestamp)).offset(skip).limit(limit).all()
...
def get_security_policy_by_id(db: Session, policy_id: int):
    return db.query(models.SecurityPolicy).filter(models.SecurityPolicy.id == policy_id).first()
...
def get_incidents(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.Incident).order_by(desc(models.Incident.reported_at)).offset(skip).limit(limit).all()
...
def get_compliance_checks(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.ComplianceCheck).order_by(desc(models.ComplianceCheck.last_check)).offset(skip).limit(limit).all()
