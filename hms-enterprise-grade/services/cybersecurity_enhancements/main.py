...
def create_security_policy(policy: schemas.SecurityPolicyCreate, db: Session = Depends(get_db)):
...
def create_access_control_rule(rule: schemas.AccessControlRuleCreate, db: Session = Depends(get_db)):
...
def read_access_control_rules(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def read_compliance_checks(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def create_encryption_key(key: schemas.EncryptionKeyCreate, db: Session = Depends(get_db)):
    return crud.create_encryption_key(db=db, key=key)

@app.get("/encryption-keys/", response_model=List[schemas.EncryptionKey])
def read_encryption_keys(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
