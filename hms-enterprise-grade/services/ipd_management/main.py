...
async def create_admission(admission: schemas.IPDAdmissionCreate, db: Session = Depends(get_db)):
...
async def get_admission(admission_id: int, db: Session = Depends(get_db)):
    admission = db.query(models.IPDAdmission).filter(models.IPDAdmission.id == admission_id).first()
    if not admission:
...
async def create_nursing_care(care: schemas.NursingCareCreate, db: Session = Depends(get_db)):
...
async def create_discharge_summary(summary: schemas.DischargeSummaryCreate, db: Session = Depends(get_db)):
