...
def get_admission_by_id(db: Session, admission_id: int):
    return db.query(models.IPDAdmission).filter(models.IPDAdmission.id == admission_id).first()
...
def get_discharge_summary_by_id(db: Session, summary_id: int):
    return db.query(models.DischargeSummary).filter(models.DischargeSummary.id == summary_id).first()
...
def get_active_admissions(db: Session):
    return db.query(models.IPDAdmission).filter(models.IPDAdmission.status == "admitted").all()
