...
from core.encryption.fields import EncryptedString, EncryptedJSON
from core.auth.dependencies import get_current_user

DATABASE_URL = os.getenv("HOUSEKEEPING_MAINTENANCE_DATABASE_URL", "postgresql+psycopg2://hms:hms@db:5432/hms")
...
def list_tasks(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ensure_role(current_user, {"admin", "housekeeping_manager"})
    return db.query(HousekeepingTask).all()

@app.post("/api/housekeeping/tasks", response_model=HousekeepingTaskOut, status_code=201)
def create_task(payload: HousekeepingTaskIn, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ensure_role(current_user, {"admin", "housekeeping_manager"})
...
def update_task(task_id: int, payload: HousekeepingTaskIn, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ensure_role(current_user, {"admin", "housekeeping_manager"})
