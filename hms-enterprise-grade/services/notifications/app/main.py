...
DATABASE_URL = os.getenv("NOTIFICATIONS_DATABASE_URL", os.getenv("DATABASE_URL", "sqlite:///./notifications.db"))
...
def send(payload: SendPayload, claims: dict = Depends(require_auth), db: Session = Depends(get_db)):
    role = claims.get('role')
    if role not in { 'SUPER_ADMIN', 'HOSPITAL_ADMIN' }:
        raise HTTPException(status_code=403, detail="Forbidden")
    row = NotificationModel(channel=payload.channel, recipient=payload.recipient, subject=payload.subject, message=payload.message, status='queued')
    db.add(row)
    db.commit()
    db.refresh(row)
    ...
    try:
        if payload.channel.lower() == 'email':
            ...
            _send_fcm(payload.recipient, payload.title if hasattr(payload, 'title') else payload.subject, payload.message)
        else:
...
def history(_: dict = Depends(require_auth), db: Session = Depends(get_db)):
    rows = db.query(NotificationModel).order_by(NotificationModel.id.desc()).limit(100).all()
    return [
        {"id": r.id, "channel": r.channel, "recipient": r.recipient, "subject": r.subject, "status": r.status, "created_at": r.created_at.isoformat(), "sent_at": r.sent_at.isoformat() if r.sent_at else None}
        for r in rows
    ]
...
def _load_secrets_from_vault() -> None:
    ...
    try:
        ...
        for key in ['SMTP_HOST','SMTP_PORT','SMTP_USER','SMTP_PASSWORD','SMTP_SENDER','SMS_PROVIDER_URL','SMS_PROVIDER_TOKEN','FCM_SERVER_KEY']:
            ...
            for msg in consumer:
                ...
                if evt.get('event') == 'appointment_created':
                    ...
                    with SessionLocal() as db:
                        row = NotificationModel(channel='email', recipient='ops@example.com', subject='New Appointment', message=json.dumps(evt), status='queued')
