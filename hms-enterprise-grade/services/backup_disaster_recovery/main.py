...
def create_backup_job(backup_job: schemas.BackupJobCreate, db: Session = Depends(get_db)):
...
def update_backup_job(job_id: int, backup_job: schemas.BackupJobCreate, db: Session = Depends(get_db)):
...
def create_backup_execution(execution: schemas.BackupExecutionCreate, db: Session = Depends(get_db)):
...
def create_recovery_job(recovery_job: schemas.RecoveryJobCreate, db: Session = Depends(get_db)):
...
def create_disaster_recovery_plan(plan: schemas.DisasterRecoveryPlanCreate, db: Session = Depends(get_db)):
...
@app.get("/disaster-recovery-plans/{plan_id}", response_model=schemas.DisasterRecoveryPlan)
...
def create_storage_configuration(config: schemas.StorageConfigurationCreate, db: Session = Depends(get_db)):
...
def get_default_storage_configuration(db: Session = Depends(get_db)):
    ...
    if not config:
        raise HTTPException(status_code=404, detail="Default storage configuration not found")
    return config
...
async def execute_backup(backup_request: schemas.BackupRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
