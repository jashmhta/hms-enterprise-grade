...
DATABASE_URL = os.getenv("CONSENT_DATABASE_URL", os.getenv("DATABASE_URL", "sqlite:///./consent.db"))
...
def on_startup():
    ...
    try:
        if db.query(TemplateModel).count() == 0:
            db.add(TemplateModel(name="General Consent", body="I hereby consent to diagnosis and treatment."))
            db.commit()
    finally:
        db.close()
...
def list_templates(claims: dict = Depends(require_auth), db: Session = Depends(get_db)):
    ensure_role(claims, { 'SUPER_ADMIN', 'HOSPITAL_ADMIN', 'DOCTOR', 'NURSE', 'RECEPTIONIST' })
    rows = db.query(TemplateModel).order_by(TemplateModel.id.asc()).all()
    return rows

@app.post("/api/consent/templates", response_model=TemplateOut)
def create_template(payload: TemplateIn, claims: dict = Depends(require_auth), db: Session = Depends(get_db)):
...
def sign(payload: SignIn, claims: dict = Depends(require_auth), db: Session = Depends(get_db)):
    ensure_role(claims, { 'SUPER_ADMIN', 'HOSPITAL_ADMIN', 'DOCTOR', 'NURSE', 'RECEPTIONIST' })
    exists = db.query(TemplateModel).filter(TemplateModel.id == payload.template_id).first()
    if not exists:
...
def list_signatures(claims: dict = Depends(require_auth), db: Session = Depends(get_db)):
    ensure_role(claims, { 'SUPER_ADMIN', 'HOSPITAL_ADMIN' })
