...
create_pg, list_pg = crud_factory(models.PricingGroup, schemas.PricingGroupCreate, schemas.PricingGroupRead)
...
create_dep, list_dep = crud_factory(models.Department, schemas.DepartmentCreate, schemas.DepartmentRead)
...
create_ri, list_ri = crud_factory(models.ReferralIncome, schemas.ReferralIncomeCreate, schemas.ReferralIncomeRead)
...
create_os, list_os = crud_factory(models.OutsourcedService, schemas.OutsourcedServiceCreate, schemas.OutsourcedServiceRead)
...
create_asset, list_asset = crud_factory(models.Asset, schemas.AssetCreate, schemas.AssetRead)
...
create_je, list_je = crud_factory(models.JournalEntry, schemas.JournalEntryCreate, schemas.JournalEntryRead)
...
create_le, list_le = crud_factory(models.LedgerEntry, schemas.LedgerEntryCreate, schemas.LedgerEntryRead)
...
def validate_double_entry(journal_entry_id: int, db: Session):
    entries = db.query(models.LedgerEntry).filter_by(journal_entry_id=journal_entry_id).all()
...
def department_pnl(db: Session = Depends(get_db)):
    data = db.query(models.Department.name, models.LedgerEntry.debit, models.LedgerEntry.credit).join(models.LedgerEntry).all()
    return data
