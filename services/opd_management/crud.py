...
def get_doctors(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.Doctor).filter(models.Doctor.is_available == True).offset(skip).limit(limit).all()
...
def get_appointments_by_doctor(db: Session, doctor_id: int, date: Optional[datetime] = None):
...
def get_doctor_availability(db: Session, doctor_id: int, date: datetime):
    ...
    for hour_range in working_hours.get('hours', []):
        ...
        current_slot = current_time.replace(hour=start_time.hour, minute=start_time.minute)
        while current_slot.hour < end_time.hour or (current_slot.hour == end_time.hour and current_slot.minute < end_time.minute):
            slot_end = current_slot + timedelta(minutes=30)
            
            # Check if slot is available
            slot_available = True
            for appointment in appointments:
                if (current_slot <= appointment.appointment_date < slot_end or
                    current_slot <= appointment.appointment_date + timedelta(minutes=appointment.duration_minutes) <= slot_end):
...
def get_opd_statistics(db: Session, date: datetime):
    ...
    today_bills = db.query(models.OPDBill).filter(
    ...
    month_bills = db.query(models.OPDBill).filter(
