...
def update_appointment_status(appointment_id: int, status: schemas.AppointmentStatus, db: Session = Depends(get_db)):
...
@app.get("/consultations/patient/{patient_id}", response_model=List[schemas.Consultation])
...
def get_doctor_availability(doctor_id: int, date: datetime, db: Session = Depends(get_db)):
