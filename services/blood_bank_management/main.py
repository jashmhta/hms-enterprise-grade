...
async def create_blood_unit(blood_unit: schemas.BloodUnitCreate, db: Session = Depends(get_db)):
...
@app.get("/blood-units/type/{blood_type}/{rh_factor}", response_model=List[schemas.BloodUnit])
async def get_blood_units_by_type(blood_type: schemas.BloodType, rh_factor: str, db: Session = Depends(get_db)):
    return crud.get_blood_units_by_type(db, blood_type, rh_factor)

@app.patch("/blood-units/{unit_id}/status")
async def update_blood_unit_status(unit_id: int, status: str, db: Session = Depends(get_db)):
...
async def create_transfusion_request(request: schemas.TransfusionRequestCreate, db: Session = Depends(get_db)):
    return crud.create_transfusion_request(db, request)

@app.patch("/transfusion-requests/{request_id}/status")
async def update_transfusion_request_status(request_id: int, status: str, approved_units: int = None, db: Session = Depends(get_db)):
    request = crud.update_transfusion_request_status(db, request_id, status, approved_units)
    if not request:
...
async def create_transfusion(transfusion: schemas.TransfusionCreate, db: Session = Depends(get_db)):
    return crud.create_transfusion(db, transfusion)

@app.post("/blood-tests/", response_model=schemas.BloodTest)
async def create_blood_test(blood_test: schemas.BloodTestCreate, db: Session = Depends(get_db)):
    return crud.create_blood_test(db, blood_test)

@app.post("/inventory-alerts/", response_model=schemas.InventoryAlert)
async def create_inventory_alert(alert: schemas.InventoryAlertCreate, db: Session = Depends(get_db)):
