...
async def create_blood_bag(blood_bag: schemas.BloodBagCreate, db: Session = Depends(get_db)):
    return crud.create_blood_bag(db=db, blood_bag=blood_bag)

@app.get("/blood-bags/available/{blood_type}/{component}", response_model=List[schemas.BloodBag])
async def get_available_blood_bags(blood_type: schemas.BloodType, component: schemas.BloodComponent, db: Session = Depends(get_db)):
...
async def create_blood_request(blood_request: schemas.BloodRequestCreate, db: Session = Depends(get_db)):
