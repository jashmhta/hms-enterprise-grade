...
def create_prescription_dispatch(db: Session, dispatch: schemas.PrescriptionDispatchCreate):
...
def get_prescription_dispatches(db: Session, prescription_id: int):
    return db.query(models.PrescriptionDispatch).filter(
        models.PrescriptionDispatch.prescription_id == prescription_id
    ).all()

def update_dispatch_status(db: Session, dispatch_id: int, status: str):
    dispatch = db.query(models.PrescriptionDispatch).filter(models.PrescriptionDispatch.id == dispatch_id).first()
    if dispatch:
...
def check_prescription_safety(db: Session, patient_id: int, medication_ids: list):
    ...
    existing_medications = [prescription.medication_id for prescription in existing_prescriptions]
