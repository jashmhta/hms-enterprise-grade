...
@app.get("/doctors/{doctor_id}/prescriptions", response_model=List[schemas.Prescription])
...
def update_prescription_status(prescription_id: int, status: str, db: Session = Depends(get_db)):
...
def create_dispatch(dispatch: schemas.PrescriptionDispatchCreate, db: Session = Depends(get_db)):
    return crud.create_prescription_dispatch(db, dispatch)

@app.get("/prescriptions/{prescription_id}/dispatches", response_model=List[schemas.PrescriptionDispatch])
...
def check_prescription_safety(request: schemas.PrescriptionCheckRequest, db: Session = Depends(get_db)):
    safety_check = crud.check_prescription_safety(db, request.patient_id, request.medication_ids)
    return safety_check

@app.post("/interactions/")
def create_interaction(interaction: schemas.DrugInteractionCreate, db: Session = Depends(get_db)):
    return crud.create_drug_interaction(db, interaction)

@app.get("/interactions/check")
def check_interactions(medication1_id: int, medication2_id: int, db: Session = Depends(get_db)):
